...
export function MobileSidebar({ language, isOpen, onClose, isDark, toggleTheme, toggleLanguage }: MobileSidebarProps) {
  ...
  useEffect(() => {
    if (isOpen && navRef.current) {
      const elements = navRef.current.querySelectorAll<HTMLElement>('.nav-item');
      
      // Reset: garante que os elementos estejam visíveis (fallback para caso animejs falhe)
      elements.forEach(el => {
        el.style.opacity = '0';
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent 
        side="left" 
        className="w-72 p-0 bg-sidebar text-foreground"
      >
        ...
        <nav 
          ref={navRef} 
          className="p-4 overflow-y-auto max-h-[calc(100vh-120px)]"
          ...
          {categories.map(category => {
            ...
            return (
              <div key={category.id} className="mb-4">
                <div className={cn(
                  "flex items-center gap-2 px-3 py-2 text-sm font-ui font-medium nav-item",
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-border bg-sidebar">
          <div className="flex items-center justify-between">
            <Button variant="ghost" size="sm" onClick={toggleLanguage} className="gap-2 text-foreground">
