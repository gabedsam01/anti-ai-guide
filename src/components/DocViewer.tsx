  ...
  return (
    <article className={cn("prose prose-neutral dark:prose-invert max-w-none", className)}>
      <ReactMarkdown
        components={{
          h1: ({ children }) => (
            <h1 className="font-display text-4xl md:text-5xl font-semibold mb-6 text-foreground">{children}</h1>
          ),
          h2: ({ children }) => (
            <h2 className="font-display text-2xl md:text-3xl font-semibold mt-10 mb-4 text-foreground border-b border-border pb-2">{children}</h2>
          ),
          h3: ({ children }) => (
            <h3 className="font-display text-xl md:text-2xl font-medium mt-8 mb-3 text-foreground">{children}</h3>
          ),
          p: ({ children }) => (
          ...
          code: ({ className, children }) => {
            ...
            if (isBlock) {
              return (
                <pre className="bg-card border border-border p-4 overflow-x-auto font-body text-sm">
